...
[⬇️ Download](../../download/Basic-Pitch-1.0.3.ablx)
...
3. A progress dialog appears while the model runs. During transcription the bar runs in indeterminate mode — the neural network is CPU-bound, so processing time scales with clip length and your machine's processing power.
...
> **Note on Arrangement clips:** audio is rendered pre-FX from the timeline before transcription. Session clips are read from their source sample. Samples stored in Ableton's compressed format can't be decoded directly from Session — move the clip to the Arrangement and convert there.

## Credits

Windows support and cross-platform path handling contributed by [@jwfeniello](https://github.com/jwfeniello).
